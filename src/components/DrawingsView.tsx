...
import { X, Upload, Loader2, Image as ImageIcon, Edit2, Trash2, ChevronDown } from 'lucide-react';
...
// 💡 도면 등록 및 수정 (로그 추가됨)
  ...
  const handleResetView = () => { setZoom(1); setViewKey(prev => prev + 1); };

  return (
    <div className="h-full p-2 md:p-4 lg:p-8 flex flex-col overflow-hidden relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div className="flex w-full gap-2">
          {/* Dropdown Type */}
          <div className="relative flex-1 md:max-w-[200px]">
            <button onClick={() => { setIsOpenType(!isOpenType); setIsOpenFloor(false); }} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-xl font-bold text-sm flex items-center justify-between gap-2">
              {selectedType}
              <ChevronDown size={16} className={`text-gray-400 transition-transform ${isOpenType ? 'rotate-180' : ''}`} />
            </button>
            {isOpenType && (
              <div className="absolute top-12 left-0 bg-white border border-gray-200 shadow-lg rounded-xl z-20 w-full p-2 space-y-1">
                {DRAWING_TYPES.map(type => (
                  <button key={type} onClick={() => { setSelectedType(type); setIsOpenType(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-sm ${selectedType === type ? 'bg-blue-50 text-blue-700' : 'hover:bg-gray-100'}`}>{type}</button>
                ))}
              </div>
            )}
          </div>
          {/* Dropdown Floor */}
          <div className="relative flex-1 md:max-w-[200px]">
            <button onClick={() => { setIsOpenFloor(!isOpenFloor); setIsOpenType(false); }} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-xl font-bold text-sm flex items-center justify-between gap-2">
              {selectedFloor}
              <ChevronDown size={16} className={`text-gray-400 transition-transform ${isOpenFloor ? 'rotate-180' : ''}`} />
            </button>
            {isOpenFloor && (
              <div className="absolute top-12 left-0 bg-white border border-gray-200 shadow-lg rounded-xl z-20 w-full p-2 space-y-1">
                {floorsForType.length > 0 ? (
                  floorsForType.map(floor => (
                    <button key={floor} onClick={() => { setSelectedFloor(floor); setIsOpenFloor(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-sm ${selectedFloor === floor ? 'bg-green-50 text-green-700' : 'hover:bg-gray-100'}`}>{floor}</button>
                  ))
                ) : (
                  <div className="px-3 py-2 text-xs text-gray-400">도면 없음</div>
                )}
              </div>
            )}
          </div>
        ...
        {(currentUser?.userRole === '골드' || currentUser?.userRole === '실버' || currentUser?.role === 'admin') && (
          <button 
            ...
            className="hidden md:block px-6 py-2 bg-blue-600 text-white rounded-lg font-bold text-sm hover:bg-blue-700 transition-colors w-full md:w-auto shadow-md shadow-blue-500/10 whitespace-nowrap"
          >
            도면추가
          </button>
        )}
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-4 lg:gap-6 overflow-hidden">
        ...
        <div className="flex-1 bg-white border border-gray-200 rounded-2xl flex items-center justify-center relative overflow-hidden p-2 md:p-3 lg:p-4">
          <div className="w-full h-full bg-white flex items-center justify-center relative overflow-hidden cursor-grab active:cursor-grabbing">
            {currentDrawing ? (
              <motion.div
                key={viewKey}
                drag
            ...
            <div className="absolute bottom-2 right-2 md:bottom-4 md:right-4 flex flex-col gap-2 z-10">
              ...
              <button onClick={handleResetView} className="w-8 h-8 md:w-10 md:h-10 bg-blue-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold shadow-lg hover:bg-blue-700 transition-all active:scale-95">1:1</button>
            </div>
            {currentDrawing && (
